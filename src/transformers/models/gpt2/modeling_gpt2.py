...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.ff = nn.Linear(config.n_embd * 2, config.n_embd)
    ...
    def get_multi_label_loss(self, labels, multi_label_logits):
        binary_tensor = torch.nn.functional.one_hot(torch.where(labels == IGNORE_INDEX, torch.tensor(0).to(labels.device), labels),
    ...
    def get_multi_label_max_pred(self, multi_label_preds):
        max_token_id = multi_label_preds.argmax(dim=2)
        return self.transformer.wte(max_token_id)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            res = torch.relu(self.ff(cat_hidden_states))
            lm_logits = self.lm_head(res)
